...
onMount(async () => {
  try {
    ...
    const [contractsRaw, erteRaw, grantsRaw] = await Promise.all([
      fetch('https://sos2425-18.onrender.com/api/v2/contr-mun-stats').then(r => r.json()),
      fetch('https://sos2425-18.onrender.com/api/v2/dana-erte-stats/loadInitialData').then(r => r.json()),
      fetch('https://sos2425-18.onrender.com/api/v2/dana-grants-subsidies-stats').then(r => r.json())
    ]);

    // 2. Agrupación por año-mes para contratos
    const byContracts = {};
    ...
    // 3. Agrupación por año-mes para ERTEs
    const byErte = {};
    erteRaw.forEach(d => {
    ...
    // 4. Agrupación por año-mes para subvenciones
    const byGrants = {};
    grantsRaw.forEach(d => {
      if (d.year && d.month != null) {
        const key = `${d.year}-${String(d.month).padStart(2,'0')}`;
        // Asegurarse de que estamos sumando las solicitudes de subvención adecuadamente.
        byGrants[key] = (byGrants[key] || 0) + 1; // Cuenta el número de solicitudes en lugar de un valor monetario
      }
    });

    // 5. Define el rango fijo de meses
    const months = ["2024-10","2024-11","2024-12","2025-01"];
    
    // 6. Filtra solo los meses con al menos un dato
